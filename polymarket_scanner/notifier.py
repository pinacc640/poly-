...
Each approved opportunity gets one Telegram HTML message:

    🚨 POLYMARKET ALERT — Stable | EV: +0.1500

    📌 Will X resolve YES by Dec 31?
       Direction : BUY YES
       Entry     : 0.2500  |  AI Prob: 0.8000
       🎯 TP      : 0.7200  (+188.0%)
       Kelly f   : +33.33%  →  ¼-Kelly = $4.17
       💵 Bet     : $4.17  |  Est. Profit: $2.50
       📖 Exec    : 🟡 MAKER @ 0.2400
...
_MAX_LEN = 4096   # Telegram hard character limit
...
def _exec_line(opp) -> str:
    advice = getattr(opp, "order_advice", None)
    if advice is None:
        return ""
    tag = "🟢 TAKER" if advice.order_type == "TAKER" else "🟡 MAKER"
    return f"\n   📖 Exec    : {tag} @ {advice.limit_price:.4f}"


def _kelly_line(dec) -> str:
    return (
        f"\n   Kelly f   : {dec.kelly_f * 100:+.2f}%  →  "
        f"¼-Kelly = ${dec.kelly_position:.2f}"
    )


def _side_from_rationale(rationale) -> str:
...
def _msg_stable(opp: StableOpportunity, pos: float, dec) -> str:
    ...
    side   = _side_from_rationale(opp.rationale)
    url    = opp.market.polymarket_url()
    return (
        f"🚨 <b>POLYMARKET ALERT</b> — Stable | EV: {opp.ev:+.4f}\n\n"
        f"📌 <b>{opp.market.question}</b>\n"
        f"   Direction : BUY {side}\n"
...
def _msg_volatility(opp: VolatilityOpportunity, pos: float, dec) -> str:
    scale  = pos / opp.suggested_position if opp.suggested_position else 1
    profit = opp.expected_profit * scale
...
def _msg_smart_money(opp: SmartMoneyOpportunity, pos: float, dec) -> str:
    scale    = pos / opp.suggested_position if opp.suggested_position else 1
    profit   = opp.expected_profit * scale
    tp       = opp.take_profit_price
    tp_str   = f"{tp:.4f}  ({_pct(opp.market.price, tp)})" if tp > 0 else "n/a"
    ...
    return (
        ...
        f"📌 <b>{opp.market.question}</b>\n"
        f"   Direction : {opp.flow_direction}  ({breakout})\n"
        f"   Entry     : {opp.market.price:.4f}  |  AI Prob: {opp.market.true_prob:.4f}\n"
        f"   Vol/Liq   : {opp.volume_spike_ratio:.2f}"
        f"  |  Impact: {opp.price_impact_ratio:.3f}"
...
class TelegramNotifier:
    ...
    send_summary :
        If True (default), prepend a summary message before individual alerts.
    """
