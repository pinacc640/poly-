...
Message format
--------------
Each approved opportunity gets one Telegram message:
...
def _kelly_line(decision) -> str:
    f_pct = decision.kelly_f * 100
    return f"\n   Kelly f   : {f_pct:+.2f}%  →  ¼-Kelly = ${decision.kelly_position:.2f}"
...
def _msg_stable(opp: StableOpportunity, pos: float, dec) -> str:
    ...
    side   = next((n.replace("side=", "") for n in opp.rationale if n.startswith("side=")), "YES")
    return (
...
def _msg_smart_money(opp: SmartMoneyOpportunity, pos: float, dec) -> str:
    ...
    return (
        ...
        f"📌 <b>{opp.market.question}</b>\n"
        f"   Direction : BUY {opp.flow_direction}  ({breakout})\n"
