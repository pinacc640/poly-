...
# ---------------------------------------------------------------------------
# Risk thresholds  (easily adjustable without touching logic)
# ---------------------------------------------------------------------------
...
def _pnl_no(size: float, avg_yes_price: float, current_yes_price: float) -> float:
    """For a NO position, profit = (1-current) token value minus entry value."""
    return round(size * ((1 - current_yes_price) - (1 - avg_yes_price)), 2)


def _url(market) -> str:
    """Return market URL — works with or without polymarket_url() method."""
...
def _check_tp(pos, market) -> Optional[str]:
    ...
    else:  # NO side — we profit when YES price falls
...
def _check_sl(pos, market) -> Optional[str]:
    ...
    else:  # NO side — we lose when YES price rises
        no_entry   = 1 - entry
...
class PositionMonitor:
    """Checks all active portfolio positions against current market data.

    Parameters
    ----------
    tp_capture_ratio : float
        Fraction of the AI-edge gap to treat as the TP target.
    stop_loss_pct : float
        Fraction of entry price lost before triggering stop loss.
    avg_down_drop_pct : float
        Fraction of entry price dropped before suggesting averaging down.
    avg_down_edge_min : float
        Minimum AI edge (true_prob - current_price) required to suggest avg down.
    """
    ...
    def check(self, portfolio: dict, markets: list) -> List[str]:
        """Evaluate all positions; return list of Telegram HTML alert strings.
        ...
        Returns
        -------
        List of alert message strings (may be empty if nothing triggered).
        """
        if not portfolio:
            log.debug("PositionMonitor: no active positions, skipping checks.")
            return []
        ...
        for mid, pos in portfolio.items():
            ...
            if market is None:
                # Market not in current scan (could be low liquidity / filtered out)
                log.debug("Position %s not in current scan data — skipped", mid)
                n_skipped += 1
                continue

            # Priority: TP > SL > Avg Down (only fire one per position per run)
            ...
            if alert:
                alerts.append(alert)
                log.info("Position alert for %s: triggered", mid)

        log.info(
            "PositionMonitor: %d alert(s) from %d position(s) (%d skipped — not in scan)",
