...
Phase 3 新增：
  - Telegram 推送（设置 TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID 环境变量）
...
--------
    ...
    DEEPSEEK_API_KEY       (同上，优先级更高)
    TELEGRAM_BOT_TOKEN     Telegram 推送 Bot Token
    TELEGRAM_CHAT_ID       Telegram 推送 Chat ID
...
def main() -> None:
    ...
    # Silently no-ops if either variable is not set.
